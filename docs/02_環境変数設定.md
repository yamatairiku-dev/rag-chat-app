...
| `APP_TITLE` | ❌ | `Difyフロントエンドアプリ` | ヘッダー左側に表示するアプリケーションタイトル |
...
| `GRAPH_DEPARTMENT_GROUP_PREFIX` | ❌ | `^ZA[A-Za-z]\d{3}-[A-Za-z]` | 部署グループの表示名にマッチさせる正規表現パターン（例: 先頭2文字ZA、3文字目英字、4–6文字目数字、7文字目ハイフン、8文字目英字） |

**部署グループのマッチ例（デフォルト正規表現）:**
- パターン: `^ZA[A-Za-z]\d{3}-[A-Za-z]`
- マッチする表示名例: `ZAA123-X`, `ZAB456-Y`（先頭2文字 ZA、3文字目英字、4–6文字目数字、ハイフン、8文字目英字）

### Dify API設定

| 変数名 | 必須 | デフォルト | 説明 |
|--------|------|-----------|------|
| `DIFY_API_URL` | ✅ | - | Dify APIのベースURL（/v1を含む） |
...
ENTRA_REDIRECT_URI=http://localhost:3000/auth  # 開発環境の場合
