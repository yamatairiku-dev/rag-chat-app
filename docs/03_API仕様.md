...
## API 動作確認用チャットフロー（Dify DSL）

リポジトリ直下の **`ちゃっとのお試し.yml`** は、バックエンドの Dify 用 DSL（ワークフロー定義）です。本アプリのチャット API の受け側を確認するためのチャットフローとして利用します。Dify にインポートし、API 連携の動作確認に使用してください。
