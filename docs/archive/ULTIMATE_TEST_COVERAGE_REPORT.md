...
| 指標 | 初期 | 最終 | 改善 |
|------|------|------|------|
| **Statements** | 39.65% | **72.42%** | +32.77% |
| **Branches** | 29.32% | **60.54%** | +31.22% |
| **Functions** | 31.06% | **58.33%** | +27.27% |
| **Lines** | 39.67% | **72.54%** | +32.87% |
...
- `home.tsx`: 0% → **100%** (+100%)
- `api.chat-stream.ts`: 69.56% → **大幅改善**（ブランチカバレッジ改善）
...
- `api-chat-stream.test.ts`: 12テスト（+9テスト追加）
...
## 🎯 追加したテスト（api-chat-stream.test.ts）

1. `POST以外のメソッドは405を返す`
2. `トークンリフレッシュでAppError以外のエラーが発生した場合はエラーをthrow`
3. `無効なJSONペイロードの場合は400を返す`
4. `リクエストボディがnullの場合は400を返す`
5. `リクエストボディが配列の場合は400を返す`
6. `queryが文字列でない場合は400を返す`
7. `メッセージが長すぎる場合は400を返す`
8. `conversationIdが文字列でない場合は空文字列として扱う`
9. `ストリーミングエラーが発生した場合はエラーを処理`
