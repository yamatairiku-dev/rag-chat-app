...
このプロジェクトでは、**カスタムセッション管理**を実装しています。

- **Express.jsに依存しない**: `express-session` ライブラリは使用していません
- **Web標準API**: React Router v7の `Request`/`Response` APIを使用
- **独自実装**: セッションIDの生成、署名、検証を自前で実装
- **軽量**: 必要最小限の機能のみを実装
