...
sequenceDiagram
    ...
    alt セッション無し
        ...
        U->>E: 認証情報入力
        E->>B: Redirect to /auth?code=xxx
        B->>A: GET /auth?code=xxx
...
2. **アプリケーションの登録**
   ...
   - **リダイレクトURI**: 
     - プラットフォーム: `Web`
     - URI: `https://your-app.azurewebsites.net/auth`
...
export async function getUserDepartment(
  ...
  const departmentGroup = response.value.find((group: any) =>
    group.displayName != null && departmentRegex.test(group.displayName)
  );
  ...
  return {
    code: departmentGroup.id,
    name: departmentGroup.displayName,
