...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  logger.debug("[認証コールバック] リクエスト受信", { code: code ? "あり" : "なし", error });

  if (error) {
    logger.error("[認証コールバック] エラー", { error });
    return redirect(
  ...
  if (!code) {
    logger.error("[認証コールバック] 認証コードが見つかりません");
    return redirect(
  ...
  try {
    logger.debug("[認証コールバック] ステップ1: トークン取得開始");
    // 1. トークン取得
    const tokens = await exchangeCodeForTokens(code);
    logger.debug("[認証コールバック] ステップ1: トークン取得完了", {
      hasAccessToken: !!tokens.accessToken,
      hasRefreshToken: !!tokens.refreshToken,
      expiresIn: tokens.expiresIn
    });

    logger.debug("[認証コールバック] ステップ2: ユーザー情報取得開始");
    // 2. ユーザー情報取得
    const userInfo = await getUserInfo(tokens.accessToken);
    logger.debug("[認証コールバック] ステップ2: ユーザー情報取得完了", {
      userId: userInfo.id,
      displayName: userInfo.displayName,
      email: userInfo.mail || userInfo.userPrincipalName
    });

    logger.debug("[認証コールバック] ステップ3: 所属部署取得開始");
    // 3. 所属部署取得（正規表現にマッチするすべてを配列で取得）
    const departments = await getUserDepartment(tokens.accessToken);

    if (departments.length === 0) {
      logger.error("[認証コールバック] 所属部署が見つかりません。詳細は上記の[所属部署取得]ログを確認してください");
      return redirect(
        `/error?title=${encodeURIComponent("アクセス権限がありません")}&message=${encodeURIComponent("所属部署が見つかりません。アクセス権限がありません。")}&status=403`
      );
    }
    logger.debug("[認証コールバック] ステップ3: 所属部署取得完了", {
      count: departments.length,
      codes: departments.map((d) => d.code),
      names: departments.map((d) => d.name),
    });

    logger.debug("[認証コールバック] ステップ4: セッション作成開始");
    // 4. セッション作成
    ...
    logger.debug("[認証コールバック] ステップ4: セッション作成完了");

    logger.debug("[認証コールバック] ステップ5: チャット画面へリダイレクト");
    // 5. チャット画面へリダイレクト
    return redirect("/chat", {
      headers: { "Set-Cookie": cookie },
    });
  } catch (err) {
    logger.error("[認証コールバック] 認証エラー", {
